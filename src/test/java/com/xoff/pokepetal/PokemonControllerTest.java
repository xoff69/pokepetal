...
pagePokemonDto.setListPokemonsDto(pokemonDtoList);
        ...
        mockMvc.perform(get("/pokemons?page=5&size=20"))
                .andExpect(status().isOk()).andExpect(content().string(containsString("Bob242")));
    ...
    public void testSaveAndUpdate() throws Exception {
        ...
        mockMvc.perform(post("/pokemons")
                        .content(asJsonString(pokemonCreate))
                                .contentType(MediaType.APPLICATION_JSON)
                                .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated());

        mockMvc.perform(put("/pokemons/"+idPokemon) .content(asJsonString(pokemonCreatedOrUpdate))
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
